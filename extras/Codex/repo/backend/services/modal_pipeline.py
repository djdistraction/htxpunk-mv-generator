...
def apply_lipsync_to_video(
    ...
    This is deliberately separate from base rendering so failed lip-sync work
    never overwrites the reviewed base video.
    """
    ...
    try:
        ...
        logger.info("[modal-lipsync] complete â†’ %s", url)
        return url
    finally:
        shutil.rmtree(workdir, ignore_errors=True)
