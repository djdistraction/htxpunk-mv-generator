...
- Branch: `codex/issue-20-workbook-shell`
- The workbook now includes an optional Lip Sync gate after Final Real Video Generation.
- Base video generation no longer runs Modal/Wav2Lip by default. Lip sync is started only by the explicit `run-lip-sync` action after the base/final video section is approved.
- Lip sync output is stored in `lipsynced_video_url` and does not overwrite `base_video_url` or `final_video_url` until the user approves the Lip Sync section.
