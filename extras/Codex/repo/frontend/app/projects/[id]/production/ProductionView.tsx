...
export default function ProductionView({ id }: { id: string }) {
  ...
  const lipsyncedVideoUrl = project.lipsynced_video_url ? mediaUrl(project.lipsynced_video_url) : ''
  const lipSyncApproved = project.section_statuses?.lip_sync?.status === 'approved'
  ...
  const approveLipSync = async () => {
    setWorking(true)
    setLocalError('')
    try {
      const updated = await api.projects.approveSection(id, 'lip_sync')
      setProject(updated)
    } catch (err: any) {
      setLocalError(err?.response?.data?.detail || err?.message || 'Could not approve lip sync.')
    } finally {
      setWorking(false)
    }
  }

  const rejectLipSync = async () => {
    const note = window.prompt('What needs to change before this lip sync can be final?') || ''
    setWorking(true)
    setLocalError('')
    try {
      const updated = await api.projects.rejectSection(id, 'lip_sync', note)
      setProject(updated)
    } catch (err: any) {
      setLocalError(err?.response?.data?.detail || err?.message || 'Could not reject lip sync.')
    } finally {
      setWorking(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-4xl mx-auto">
        ...
        {lipsyncedVideoUrl && !lipSyncApproved && (
          <div className="mb-10 bg-gray-900 rounded-xl overflow-hidden border border-blue-800">
            <div className="p-4 border-b border-gray-800 flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                <h2 className="font-semibold text-blue-300">Lip-Synced Candidate</h2>
                <p className="text-gray-500 text-sm mt-1">
                  Review this separately. Approving it replaces the final export; rejecting it leaves the approved base video alone.
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <button
                  onClick={rejectLipSync}
                  disabled={working}
                  className="text-sm border border-orange-800 text-orange-300 hover:border-orange-600 disabled:text-gray-600 px-4 py-1.5 rounded-lg transition-colors"
                >
                  Reject Lip Sync
                ...
                <button
                  onClick={approveLipSync}
                  disabled={working}
                  className="text-sm bg-green-700 hover:bg-green-600 disabled:bg-gray-800 disabled:text-gray-500 px-4 py-1.5 rounded-lg transition-colors"
                >
                  {working ? 'Approving...' : 'Approve Lip Sync as Final'}
                </button>
              </div>
            </div>
            <video
