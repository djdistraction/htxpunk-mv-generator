...
export default function ElementsPage() {
  ...
  const fetchAssets = async () => {
    try {
      const all = await api.assets.list(id as string)
      setAssets(all)
    } finally {
      setLoading(false)
    }
  ...
  const handleRegenerate = async (assetId: string, currentPrompt: string) => {
    const newPrompt = prompt('Edit the generation prompt:', currentPrompt)
    if (!newPrompt) return
    setRegenerating(assetId)
    try {
      await api.pipeline.regenerateImage(id as string, { asset_id: assetId, new_prompt: newPrompt })
  ...
  return (
    <div className="min-h-screen bg-black text-white p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-gray-500 mb-6">Review backgrounds and character elements. Regenerate any that aren't right.</p>
                ...
                <div className="aspect-square bg-gray-800 relative">
                  {asset.url ? (
                    <img
                      ...
                      style={{ background: asset.asset_type === 'element' ? 'repeating-conic-gradient(#333 0% 25%, #222 0% 50%) 0 0 / 16px 16px' : '#111' }}
                    />
                  ) : (
